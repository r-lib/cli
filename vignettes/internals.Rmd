...
* [ ] `cli_bullets()`: `text` can be a component.
* [ ] `cli_code()`: `lines` can be a component, and also `...` can contain
  components.
* [x] `cli_div()` does not take text.
...
* [x] `cli_par()` does not take text currently, so we skip it now.
  We might have a `cli_pars()` function later, that can create multiple
  paragraphs.
* [x] `cli_rule()`: cannot use block components currently, only inline.
* [ ] `cli_text()`: `...` are components.
* [ ] `cli_ul()`: `items` can be components.
* [ ] `cli_verbatim()`: `...` are components.
