...
These are non-semantic functions to format or output content to the console.
They do not use the cli theme.

# Rules

`rule()` creates a horizontal rule, potentially with labels.
If you are building a semantic CLI, then consider using `cli_rule()` instead.
...
An optional third column may contain custom labels.
These can be colored as well:
