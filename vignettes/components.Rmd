...
Some components are block components, other are inline components. The current inline components are `cpt_span()` and `cpt_txt()`.
...
`cpt_txt()` is a special inline component, that may embed text pieces and `cpt_span()` components.

Subcomponents of `cpt_txt()` are created automatically, from a template representation that supports interpreted string literals. The notation extends that of the glue package.

```{r}
cpt_txt("Some numbers: {1:5}.")
```

In addition to the glue-style braced R expressions, cli supports pluralizatiion and styling:

```{r}
n <- 1; cpt_txt("Some letters{?s}: {letters[1:n]}.")
n <- 5; cpt_txt("Some letters{?s}: {letters[1:n]}.")
```

You can add style information to a component, except for `cpt_txt()`:

```{r}
cpt_div(cpt_txt("RED ALERT"), attr = list(style = list(color = "red")))
```

You can add one or more classes to a component (but not for \`cpt_txt()):

```{r}
cpt_div(cpt_txt("..."), attr = list(class = "divclass"))
```

You can add an id to a component:

```{r}
cpt_div(cpt_txt("..."), attr = list(id = "cpt1"))
```
...
-   [x] `padding-top`
-   [x] `text-decoration`
-   [ ] `white-space` (only normal and pre)
