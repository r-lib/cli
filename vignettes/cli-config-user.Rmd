...
control sequences. Set to `false` (case insensitive) to assume a non-ANSI
terminal. The `cli.ansi` option, if set, takes precedence.
See [is_ansi_tty()].
...
Set to `FALSE` to assume a non-ANSI terminal. This option takes precedence
over the `R_CLI_ANSI` environment variable.
See [is_ansi_tty()].
...
ANSI-capable terminals. Defaults to `TRUE`. Set to `FALSE` to show only the
most recently updated bar on a single line. Has no effect on non-ANSI
terminals, which always show a single bar.
See [is_ansi_tty()] and [cli_progress_bar()].
