...
By default a new progress bar in the same calling function terminates the
previous one (the "current" bar of that environment). To keep several bars
alive at the same time, pass `current = FALSE` and keep the ids around so
you can update and finish them explicitly:

```{asciicast}
#| label: multiple-setup
#| include: false
...
#|   once per processed file, and an inner per-file bar that resets for
#|   each file.
#| asciicast_at: "all"
#| asciicast_rows: 2
f <- function() {
...
#| label: iterator
#| fig-alt: >
#|   Example of an `iterator` progress bar, from left to right it contains
#|   a label (`Data cleaning`), a progress bar, the progress percentage,
#|   and the ETA.
#| echo: false
...
#|   unknown. From left to right it contains a spinner, the specified
#|   label ('Finding data files`), the number of tasks completed, how
#|   long it taks to complete a task, and the elapsed time.
#| echo: false
cli_progress_demo(
...
#|   a label ('Downloading`), an actual progress bar, the completed and
#|   the total download size and the ETA.
#| echo: false
cli_progress_demo(
...
#|   progress bar where empty squares are filled up, the progress
...
#| fig-alt: >
#|   Example of a customized progress bar. Centered black dots are
#|   replaced by yellow stars in the progress bar, that also has the
#|   progress percentage and the ETA.
#| asciicast_at: "all"
...
#| fig-alt: >
#|   A custom progress bar, it has two spinners, one on the left, the
#|   other on the right. In the middle it has a dynamic label that
#|   iterates over the letters of the English alphabet.
#| asciicast_at: "all"
f <- function() {
  cli_progress_bar(
    total = 26,
