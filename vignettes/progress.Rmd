...
#|   the bar with green squares, the progress percentage, and the ETA.
clean <- function() {
...
#|   has a spinner, the specified label, shows how many units are done,
#|   how many units are completed per second and the elapsed time.
walk_dirs <- function() {
...
#|   up as an 'i' (info) step first, and stays like that while it is
#|   running. When it is done, the 'i' is turned into a tick mark, and
#|   the running time of the step is added to the line at the end, in
#|   grey.
f <- function() {
...
#| fig-alt: >
#|   First the 'About to start..'. message is shown in its own line. Then
#|   a progress bar starts on the next line. The progress bar takes 10
#|   steps. After the fifth step, the progress bar is overwritten with
#|   the 'Already half way!' message and the progress bar is moved down
#|   to the third line.
f <- function(n = 10) {
...
#|   running, its line has a spinner. Once it is done, the spinner turns
#|   into a tick mark and the running time of the step is added to its
#|   line at the end.
f <- function() {
...
#| label: step-error
#| fig-alt: >
#|   Two steps are shown, each in its own line. While a step is running
#|   its line is an 'i' (info) line. The first step finishes successfully
#|   and its 'i' mark is turned into a tick mark. The second step fails
#|   with an error and its line is overwritten with the error message,
#|   and moved down to the third line, and marked with an 'x' (error)
#|   mark.
#| asciicast_rows: 3
f <- function() {
