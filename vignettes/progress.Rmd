...
have to terminate the progress bar manually:
...
Often you don't need the full power of a progress bar, but only want to
show a status message. The `cli_progress_set_status()` and
`cli_progress_step()` functions are taylored for this.

`cli_progress_set_status()` shows a (potentially dynamic) message in the
...
* it adds cli's alert themes to the status messages, and
